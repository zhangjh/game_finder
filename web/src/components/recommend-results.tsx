  ...
  if (result.items.length === 0) {
    return (
      <div className="mt-8 rounded-xl border border-dashed border-border p-8 text-center text-muted">
        {result.parsedOk ? t("recNoMatch") : t("recNotParsed")}
        <div className="mt-3 flex flex-wrap justify-center gap-2">
          {QUICK_CONDITIONS.slice(0, 3).map((q) => (
            ...
            >
              {q.icon} {lang === "en" ? q.labelEn : q.label}
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2 text-xs text-muted">
        {result.referenceGame && (
          <span className="rounded-full bg-primary/10 px-3 py-1 text-primary">
            {t("recBasedOn", {
              title: displayTitle(result.referenceGame, lang),
            })}
          </span>
        )}
        {result.relaxed && (
        ...
        <span>{t("recPicked", { n: result.items.length })}</span>
      </div>
