...
/**
 * 游戏启动区（PRD §31）：
 * - 首次点击「开始游戏」后才加载 iframe（省流量、快首屏）
 * - 加载中 / 失败态处理；M6 将在此挂 behavior 埋点（start / 30s / 2min / 5min / exit）
 */
export function GamePlayer({
  ...
  useEffect(() => {
    if (!playing) return;
    const t = setTimeout(() => {
      // about:blank 等占位 URL 不会触发 onError，这里仅作为
      // 真实源上线后 iframe 加载超时的兜底逻辑占位（M3 接真源后校准）
