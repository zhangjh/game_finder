...
- Google Search Console 验证通过 + sitemap 已提交

**里程碑验收：** 生产环境通过全部 E2E Case；6 小时后首次自动同步成功；看板有真实数据；Search Console 显示 sitemap 已接收。

---

## M8 盈利模式接入（流量验证后）

### T8.1 [P0] Google AdSense 广告变现

- 申请 Google AdSense 账号并配置 `ads.txt`
- 首页 / 游戏列表页 / 游戏详情页添加广告位（Banner / 信息流）
- 广告展示策略：不影响用户体验，游戏启动页不放广告
- 追踪指标：CTR / RPM / eCPM

### T8.2 [P0] 联盟营销（Affiliate）

- 游戏手柄/外设推荐页：关联 Amazon/京东联盟链接
- 相关游戏专题页底部添加"装备推荐"模块
- 追踪点击与转化

### T8.3 [P1] 广告效果监控

- Admin 后台添加广告数据看板：AdSense 收入 / 联盟佣金 / 广告位 CTR
- A/B 测试不同广告位布局对用户体验的影响

### T8.4 [P2] 付费去广告模式（后续评估，MVP 明确不做）

**背景（2026-09 实测）：** GamePix 播放器依赖其广告栈初始化后才能启动游戏；移动端浏览器（Edge 跟踪防护 / 广告拦截 / 第三方 Cookie 隔离）在三方 iframe 内拦截广告脚本时，播放器判定「广告被拦截」并永久停在加载态（所有游戏均受影响，PC 不受影响）。当前前台已做「启动停顿 15s 提示 + 重新加载 + 新标签页打开」兜底，纯提示不做绕过。

**目标：** 提供付费开通「去广告模式」，让付费用户的嵌入游戏在移动端无拦截直接可玩。

**技术铺垫（已确认可行）：** 播放器内置「家长管理广告」模式——嵌入 URL 加 `gpxParentManager=1`（或 `gpxParentInterstitial=1`）后，播放器跳过广告脚本加载、不再触发 `adBlockerDetected`，游戏立即启动。即用即弃，一行参数即可在 `web/src/api.ts` 的 `withExternalSave` 里按账号维度追加。

**阻塞项：**
- GamePix 商务确认：该参数会关闭该嵌入的全部广告展示，需确认自家接入协议是否允许「无广告 / 自主变现」档位（主动绕过变现可能封号/断流）。
- 付费/账号基建：当前 MVP 明确不做会员体系（PRD §39/50 红线），需支付（如 Stripe/Razorpay/微信）与我们自己的鉴权（如可签发短期去广告 token 的服务端逻辑）+ cookie 短 token 校验，随外链保存标记。

**验收（后续实现时）：** 已付费用户在移动端带拦截器也能直接开玩；未付费用户维持现有提示兜底；去广告状态可随时撤销且不泄露到客户端可伪造。

**里程碑验收：** 广告位上线后 7 天内有首次收入；联盟链接可追踪。
...
- 在以下平台发布首批内容：
  - 知乎：写"AI 如何帮你找到想玩的游戏"专栏文章
  - 小红书：游戏推荐类笔记（适合手机游戏）
  - B站：录制 AI 推荐游戏的演示视频
  - 贴吧：在游戏相关贴吧软性推荐（避免硬广）
- 每篇内容植入 2-3 个游戏链接，追踪 UTM 来源

### T9.3 [P1] 外链建设

- 在游戏评测、游戏推荐类博客提交 guest post
- 在 36kr、少数派等科技媒体投稿"AI + 游戏推荐"概念
- 与游戏开发者合作，让他们在自己的游戏页面添加你的站点链接

### T9.4 [P1] 社交媒体账号

- 微信公众号：定期推送"本周好玩游戏推荐"
- 微博/小红书：游戏推荐内容运营
- 分享功能：游戏详情页添加"分享到微信/微博"按钮

### T9.5 [P2] 付费推广测试

- Google Ads：搜索广告（关键词：在线游戏、网页游戏）
- 信息流广告：穿山甲/腾讯广告（小预算测试 ROI）
- 设置转化追踪，确保付费流量质量
...
- Admin 后台新增流量看板：
  ...
  - 热门入口页面
  - 用户留存曲线（D1/D3/D7）
  - AI 推荐 vs 传统搜索的用户行为对比
- 每周复盘，调整运营策略

**里程碑验收：** 上线后 7 天内完成搜索引擎提交；30 天内日均 UV > 100；首条社交平台内容发布。
...
PRD 第 39/50 章红线：无微服务、无 Redis/ES/Kafka、无用户账号/评论/社区、无会员、无 App/小程序。付费去广告（T8.4）同样归入「后续评估」：需 GamePix 商务确认无广告档位 + 支付/账号基建，MVP 期间不做。任何时候 tempted，回到 PRD 第 55 章原则：**时间只花在"让用户更快找到想玩的游戏"上。**
